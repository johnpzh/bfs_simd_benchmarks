...
int main( int argc, char** argv) 
{
	...
	if(argc > 5) {
		...
		min_buffer_size = strtoul(argv[4], NULL, 0); 
		max_buffer_size = strtoul(argv[5], NULL, 0); 
	} else {
		printf("Usage: ./bfs <data> <tile_size> <stripe_length> <min_buffer_size> <max_buffer_size>\n");
		exit(1);
	}
	...
	T_RATIO = 20;
	CHUNK_SIZE = 2048;
	for (SIZE_BUFFER_MAX = min_buffer_size;
			SIZE_BUFFER_MAX <= max_buffer_size;
			SIZE_BUFFER_MAX *= 2) {
	//printf("tile_size: %u\n", TILE_WIDTH);
