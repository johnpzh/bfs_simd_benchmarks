...
for ((i = 1; i < 513; i *= 2)); do
	...
	#amplxe-cl -collect memory-access -result-dir ${report_dir} -data-limit=0 -knob analyze-mem-objects=true -knob analyze-openmp=true -- numactl -m 0 ./page_rank ${data_file} 65536 ${i}
	#amplxe-cl -report summary -result-dir ${report_dir} -format text -report-output ${report_dir}/report.txt
	numactl -m 0 ./page_rank ${data_file} 65536 ${i} &>> $output
done
