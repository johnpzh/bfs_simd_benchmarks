...
#pragma omp parallel for num_threads(NUM_THREADS)
	for (unsigned i = 0; i < num_tiles; ++i) {
		...
		for (; j < top - NUM_P_INT; j += NUM_P_INT) {
			...
			if (*((short *)(&is_conflict))) {
				unsigned *conflicts = (unsigned *) (&conflict_n2);
				float *sums = (float *) (&sum_n2_v);
				for (unsigned k = 0; k < NUM_P_INT; ++k) {
					unsigned n2_c = conflicts[k];
					if (!n2_c) {
						continue;
					}
					unsigned n1 = tiles_n1[i][j + k];
					unsigned n2 = tiles_n2[i][j + k];
					float sum_orig = sums[k];
					sum_orig += rank[n1]/grah.nneibor[n1];
					sum[n2] = sum_orig;
				}
			}
			//} else {
