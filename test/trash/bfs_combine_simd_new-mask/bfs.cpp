...
#pragma omp parallel for reduction(+: out_degree)
	...
	while (frontier_size != 0) {
		if (frontier_size + out_degree > bfs_threshold) {
			...
			last_time = omp_get_wtime();
			h_updating_graph_mask = (int *) calloc(NNODES, sizeof(int));
			BFS_dense(
					h_graph_heads,
					h_graph_tails,
					h_graph_mask,
		...
		if (last_is_dense) {
			frontier_size = 0;
			out_degree = 0;
			int *new_h_graph_mask = (int *) malloc(NNODES * sizeof(int));
#pragma omp parallel for reduction(+: frontier_size, out_degree)
			for (unsigned side_id = 0; side_id < SIDE_LENGTH; ++side_id) {
				if (!is_updating_active_side[side_id]) {
					...
					memset(new_h_graph_mask + side_id * TILE_WIDTH, 0, width * sizeof(unsigned));
					continue;
				}
				...
				for (vertex_id = start_vertex_id; 
					...
					if (!is_updating_m) {
						_mm512_storeu_si512(new_h_graph_mask + vertex_id, _mm512_set1_epi32(0));
						continue;
					}
					_mm512_mask_storeu_epi32(h_updating_graph_mask + vertex_id, is_updating_m, _mm512_set1_epi32(0));
					_mm512_storeu_si512(new_h_graph_mask + vertex_id, updating_flag_v);
