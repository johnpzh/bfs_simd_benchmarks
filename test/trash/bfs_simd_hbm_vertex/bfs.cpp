...
unsigned *to_sparse(
		...
		int *h_graph_mask)
{
	//unsigned *new_frontier = (unsigned *) hbw_malloc(frontier_size * sizeof(unsigned));
	unsigned *new_frontier;
	hbw_posix_memalign((void **) &new_frontier, ALIGNED_BYTES, frontier_size * sizeof(unsigned));
	...
	if (num_blocks > 1) {
		nums_in_blocks = (unsigned *) hbw_malloc(num_blocks * sizeof(unsigned));
				...
				unsigned &queue_size)
{
	...
	//unsigned *degrees = (unsigned *) hbw_malloc(sizeof(unsigned) *  queue_size);
	unsigned *degrees;
	hbw_posix_memalign((void **) &degrees, ALIGNED_BYTES, sizeof(unsigned) * queue_size);
...
//	}
	...
	//time_now = omp_get_wtime();
	//unsigned *new_frontier_tmp = (unsigned *) hbw_malloc(sizeof(unsigned) * new_queue_size);
	unsigned *new_frontier_tmp;
	hbw_posix_memalign((void **) &new_frontier_tmp, ALIGNED_BYTES, sizeof(unsigned) * new_queue_size);
#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
	...
	nums_in_blocks = (unsigned *) hbw_malloc(sizeof(unsigned) * num_blocks);
...
#pragma omp parallel for reduction(+: new_frontier_size_tmp)
	...
	//time_now = omp_get_wtime();
	//unsigned *new_frontier = (unsigned *) hbw_malloc(sizeof(unsigned) * new_queue_size);
	unsigned *new_frontier;
	hbw_posix_memalign((void **) &new_frontier, ALIGNED_BYTES, sizeof(unsigned) * new_queue_size);
	if (num_blocks > 1) {
	//TODO: blocked parallel for
	//double time_now = omp_get_wtime();
	offset_sum = 0;
		...
		const unsigned &source)
{
	...
	int *h_graph_mask = (int*) hbw_malloc(sizeof(int)*NNODES);
	int *h_updating_graph_mask = (int*) hbw_malloc(sizeof(int)*NNODES);
	int *h_cost = (int*) hbw_malloc(sizeof(int)*NNODES);
	...
	unsigned *h_graph_parents = (unsigned *) hbw_malloc(sizeof(unsigned) * NNODES);
...
#pragma omp parallel for num_threads(64)
	...
	//unsigned *frontier = (unsigned *) hbw_malloc(sizeof(unsigned) * frontier_size);
	unsigned *frontier = nullptr;
	hbw_posix_memalign((void **) &frontier, ALIGNED_BYTES, sizeof(unsigned) * frontier_size);
	frontier[0] = source;
	//// PAPI
	...
	unsigned out_degree = update_sparse(
								frontier,
								frontier_size,
								graph_degrees,
								h_graph_parents,
...
#pragma omp parallel
{
	...
	hbw_free(frontier);
	hbw_free( h_graph_mask);
	hbw_free( h_updating_graph_mask);
			...
			unsigned *&graph_degrees)
{
	...
	tile_sizes = (unsigned *) hbw_malloc(NUM_TILES * sizeof(unsigned));
	...
	graph_degrees = (unsigned *) hbw_malloc(sizeof(unsigned) * NNODES);
	graph_vertices = (unsigned *) hbw_malloc(sizeof(unsigned) * NNODES);
