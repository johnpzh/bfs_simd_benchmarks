...
inline void scheduler_dense_reverse_one(
		...
		float *dependencies)
{
	for (unsigned row_id = 0; row_id < SIDE_LENGTH; ++row_id) {
		unsigned start_tile_id = row_id * SIDE_LENGTH + start_col_index;
		...
		for (unsigned tile_id = start_tile_id; tile_id < bound_tile_id; ++tile_id) {
			...
			if (!tile_sizes[tile_id] || !is_active_side[col_id]) {
				continue;
			}
			// Kernel
			unsigned bound_edge_i;
		...
		float *dependencies)
{
	...
	for (unsigned side_id = 0; side_id < bound_column_id; side_id += ROW_STEP) {
		scheduler_dense_reverse_one(
				//side_id * SIDE_LENGTH,\
				//(side_id + ROW_STEP) * SIDE_LENGTH,
				side_id,
				ROW_STEP,
