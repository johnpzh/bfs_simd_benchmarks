...
unsigned T_RATIO;
unsigned WORK_LOAD;
unsigned NUM_FULL_STEP_TILE;
unsigned NUM_FULL_STEP_SIDE;
		...
		//unsigned *&tile_sizes_reverse)
{
	...
	NUM_FULL_STEP_SIDE = (SIDE_LENGTH / ROW_STEP) * ROW_STEP;
	NUM_FULL_STEP_TILE = NUM_FULL_STEP_SIDE * SIDE_LENGTH;
...
void update_visited_dense_reverse(
		unsigned *h_graph_mask,
		int *h_graph_visited,
		int *is_active_col_side,
		float *dependencies,
		float *inverse_num_paths)
{
	memset(is_active_col_side, 0, SIDE_LENGTH * sizeof(int));
//#pragma omp parallel for schedule(dynamic, CHUNK_SIZE_DENSE)
//	for (unsigned i = 0; i < NNODES; ++i) {
//		if (h_graph_mask[i]) {
//			h_graph_visited[i] = 1;
//			is_active_col_side[i/TILE_WIDTH] = 1;
//			dependencies[i] += inverse_num_paths[i];
//		}
//	}
...
#pragma omp parallel for schedule(dynamic, CHUNK_SIZE_DENSE)
	for (unsigned i = 0; i < bound_i; i += NUM_P_INT) {
		...
		_mm512_mask_i32scatter_epi32(is_active_col_side, is_set_v, side_id_v, _mm512_set1_epi32(1), sizeof(int));
	}
	if (remainder) {
		__mmask16 in_m = (__mmask16) ((unsigned short) 0xFFFF >> (NUM_P_INT - remainder));
...
inline void scheduler_dense_reverse(
		const unsigned &start_column_index,
		const unsigned &tile_step,
		unsigned *h_graph_heads,
		unsigned *h_graph_tails,
		unsigned *heads_buffer,
...
//#pragma omp parallel for schedule(dynamic, 1)
	for (unsigned row_id = 0; row_id < SIDE_LENGTH; ++row_id) {
		unsigned start_tile_id = row_id * SIDE_LENGTH + start_column_index;
		unsigned bound_tile_id =  start_tile_id + tile_step;
		unsigned tid = omp_get_thread_num();
		unsigned *heads_buffer_base = heads_buffer + tid * SIZE_BUFFER_MAX;
		unsigned *tails_buffer_base = tails_buffer + tid * SIZE_BUFFER_MAX;
		unsigned size_buffer = 0;
		unsigned capacity = SIZE_BUFFER_MAX;
		for (unsigned tile_id = start_tile_id; tile_id < bound_tile_id; ++tile_id) {

			//unsigned col_id;
			//if (row_id < NUM_FULL_STEP_SIDE) {
			//	col_id = tile_id / tile_step % SIDE_LENGTH;
			//} else {
			//	col_id = (tile_id - NUM_FULL_STEP_TILE)  / tile_step;
			//}

			//if (0 == tile_sizes[tile_id] || !is_active_col_side[col_id]) {
			//	continue;
			//}
		...
		float *dependencies)
{
	...
	unsigned remainder = SIDE_LENGTH % ROW_STEP;
	unsigned bound_column_id = SIDE_LENGTH - remainder;
	for (unsigned column_id = 0; column_id < bound_column_id; column_id += ROW_STEP) {
