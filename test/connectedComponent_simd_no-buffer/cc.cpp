		...
		unsigned *&tile_sizes) 
{
	...
	fclose(fin);
	graph_heads = (unsigned *) _mm_malloc(NEDGES * sizeof(unsigned), ALIGNED_BYTES);
	graph_ends = (unsigned *) _mm_malloc(NEDGES * sizeof(unsigned), ALIGNED_BYTES);
	tile_sizes = (unsigned *) malloc(NEDGES * sizeof(unsigned));
	for (unsigned i = 0; i < NUM_TILES; ++i) {
		if (NUM_TILES - 1 != i) {
...
//#pragma omp parallel for
	...
	for (edge_i = edge_i_start; edge_i < bound_i; edge_i += NUM_P_INT) {
		__m512i head_v = _mm512_loadu_si512(graph_heads + edge_i);
		__m512i end_v = _mm512_loadu_si512(graph_ends + edge_i);
	...
	if (remainder > 0) {
		...
		__m512i head_v = _mm512_mask_loadu_epi32(_mm512_undefined_epi32(), in_range_m, graph_heads + edge_i);
		__m512i end_v = _mm512_mask_loadu_epi32(_mm512_undefined_epi32(), in_range_m, graph_ends + edge_i);
...
#pragma omp parallel for schedule(dynamic, 1)
	for (unsigned col_id = 0; col_id < SIDE_LENGTH; ++col_id) {
		...
		for (unsigned row_id = start_row_index; row_id < bound_row_index; ++row_id) {
			...
			unsigned *tails_start = graph_ends + tile_offsets[tile_id];
			cc_kernel(
					heads_start,
					tails_start,
					graph_active, 
